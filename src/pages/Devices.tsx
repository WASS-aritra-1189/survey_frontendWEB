...
import { Smartphone, Battery, MapPin, Loader2, Plus, Eye, Pencil, UserPlus, UserMinus, Trash2, Download, FileText } from "lucide-react";
...
export default function Devices() {
  ...
  const fetchZones = async () => {
    ...
    try {
      const zonesData = await zoneService.getAll(tokens.accessToken, { limit: 1000 });
      const list = Array.isArray(zonesData) ? zonesData : (zonesData?.data ?? []);
      setZones(list);
    } catch (error) {
      setZones([]);
    }
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" className="gap-1.5" disabled={devices.length === 0}
                onClick={() => exportToCSV(
                  devices.map(d => ({ deviceName: d.deviceName, deviceId: d.deviceId, status: d.status, battery: d.battery + '%', location: d.location, assignedTo: d.surveyMaster?.loginId || 'Unassigned' })),
                  'devices', ['deviceName','deviceId','status','battery','location','assignedTo'], 'Devices'
                )}>
                <Download className="h-4 w-4" /> CSV
              </Button>
              <Button variant="outline" size="sm" className="gap-1.5" disabled={devices.length === 0}
                onClick={() => exportReportToPDF('Devices Report', [{
                  heading: 'All Devices',
                  rows: [
                    ['Device Name', 'Device ID', 'Status', 'Battery', 'Location', 'Assigned To'],
                    ...devices.map(d => [d.deviceName, d.deviceId, d.status, d.battery + '%', d.location, d.surveyMaster?.loginId || 'Unassigned'])
                  ]
                }], 'devices', 'Devices')}>
