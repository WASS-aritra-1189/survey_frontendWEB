  ...
  const handleCreate = async () => {
    ...
    if (tokens?.accessToken) {
      try {
        ...
        setFormData({ loginId: "", password: "", surveyLimit: 100, settingId: settings[0]?.id || "" });
      } catch (error) {
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <Button variant="outline" size="sm" className="gap-1.5" disabled={surveyMasters.length === 0}
                onClick={() => exportToCSV(
                  filteredMasters.map(m => ({ loginId: m.loginId, status: m.status, surveysAssigned: m.totalSurveysAssigned, surveyLimit: m.surveyLimit, responses: m.totalResponsesGiven })),
                  'users', ['loginId','status','surveysAssigned','surveyLimit','responses'], 'Users'
                )}>
                <Download className="h-4 w-4" /> CSV
              </Button>
              <Button variant="outline" size="sm" className="gap-1.5" disabled={surveyMasters.length === 0}
                onClick={() => exportReportToPDF('Users Report', [{
                  heading: 'Survey Masters',
                  rows: [
                    ['Login ID', 'Status', 'Surveys Assigned', 'Survey Limit', 'Responses'],
                    ...filteredMasters.map(m => [m.loginId, m.status, String(m.totalSurveysAssigned), String(m.surveyLimit), String(m.totalResponsesGiven)])
                  ]
                }], 'users', 'Users')}>
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4 py-4">
                    ...
                    <div className="space-y-2">
                      <Label>Setting</Label>
                      <Input
                        value={settings.find(s => s.id === formData.settingId)?.title || formData.settingId || 'Loading...'}
                        readOnly
          ...
          {editData && (
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                <Label>Setting</Label>
                <Input
                  value={settings.find(s => s.id === editData.settingId)?.title || editData.settingId || ''}
                  readOnly
                  className="bg-muted cursor-not-allowed"
                />
              </div>
