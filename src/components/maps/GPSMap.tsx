...
export function GPSMap({
  ...
  // Auto-center on first marker/fence if no explicit center given
  const autoCenter = markers.length > 0
  ...
  useEffect(() => {
    if (markers.length > 0) setViewCenter({ lat: markers[0].lat, lng: markers[0].lng });
    else if (geoFences.length > 0) setViewCenter({ lat: geoFences[0].lat, lng: geoFences[0].lng });
  }, [markers.length, geoFences.length]);
  ...
  const refreshing = isRefreshingExternal || isRefreshing;
