...
export const surveyMasterService = {
  getAll: async (limit: number = 15, page: number = 1, token: string): Promise<SurveyMastersResponse> => {
    ...
    if (!response.ok) {
      throw new Error(extractApiError(await response.json().catch(() => null), "Failed to fetch survey masters"));
    }
  ...
  getById: async (id: string, token: string) => {
    ...
    if (!response.ok) {
      throw new Error(extractApiError(await response.json().catch(() => null), "Failed to fetch survey master"));
    }
  ...
  create: async (data: { loginId: string; password: string; surveyLimit: number; settingId?: string }, token: string) => {
    ...
    if (!response.ok) {
      throw new Error(extractApiError(await response.json().catch(() => null), "Failed to create survey master"));
    }
  ...
  updateStatus: async (id: string, status: string, token: string) => {
    ...
    if (!response.ok) {
      throw new Error(extractApiError(await response.json().catch(() => null), "Failed to update status"));
    }
  ...
  delete: async (id: string, token: string) => {
    ...
    if (!response.ok) {
      throw new Error(extractApiError(await response.json().catch(() => null), "Failed to delete survey master"));
    }
  ...
  update: async (id: string, data: { loginId?: string; password?: string; surveyLimit?: number; settingId?: string }, token: string) => {
    ...
    if (!response.ok) {
      throw new Error(extractApiError(await response.json().catch(() => null), "Failed to update survey master"));
    }

    return response.json();
  },

  resetPassword: async (id: string, data: { newPassword: string; confirmPassword: string; oldPassword?: string }, token: string) => {
