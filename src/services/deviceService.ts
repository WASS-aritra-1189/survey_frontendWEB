...
export interface DeviceLocation {
  ...
  status: string;
  battery: number;
  location: { latitude: number | null; longitude: number | null; raw: string | null };
  zone: {
    id: string;
    name: string;
    latitude: number;
...
export const deviceService = {
  getAll: async (token: string, limit: number = 100, page: number = 1): Promise<DeviceListResponse> => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to fetch devices"));
    }
  ...
  create: async (token: string, data: CreateDeviceData): Promise<{ success: boolean }> => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to create device"));
    }
  ...
  getById: async (token: string, id: string): Promise<DeviceResponse> => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to fetch device"));
    }
  ...
  update: async (token: string, id: string, data: Partial<CreateDeviceData>): Promise<{ success: boolean }> => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to update device"));
    }
  ...
  updateStatus: async (token: string, id: string, status: string): Promise<{ success: boolean }> => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to update device status"));
    }
  ...
  assignDevice: async (token: string, id: string, surveyMasterId: string): Promise<{ success: boolean }> => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to assign device"));
    }
  ...
  unassignDevice: async (token: string, id: string): Promise<{ success: boolean }> => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to unassign device"));
    }
  ...
  delete: async (token: string, id: string): Promise<{ success: boolean }> => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to delete device"));
    }

    return result;
  },

  getLocations: async (token: string): Promise<DeviceLocation[]> => {
    const response = await fetch(`${BaseUrl}/devices/locations`, {
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
    });
    const result = await response.json();
    if (!response.ok || !result.success) throw new Error(extractApiError(result, "Failed to fetch device locations"));
    return result.data as DeviceLocation[];
  },
};
