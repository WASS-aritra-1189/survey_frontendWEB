...
export const surveyService = {
  getAll: async (token: string, limit: number = 50, page: number = 1): Promise<SurveyListResponse> => {
    ...
    if (!response.ok) {
      ...
      throw new Error(extractApiError(err, "Failed to fetch surveys"));
    }
  ...
  getById: async (token: string, id: string): Promise<SurveyResponse> => {
    ...
    if (!response.ok) {
      const err = await response.json().catch(() => null);
      throw new Error(extractApiError(err, "Failed to fetch survey"));
    }
  ...
  create: async (data: Survey, token: string): Promise<SurveyResponse> => {
    ...
    if (!response.ok) {
      const err = await response.json().catch(() => null);
      throw new Error(extractApiError(err, "Failed to create survey"));
    }
  ...
  update: async (id: string, data: Partial<Survey>, token: string): Promise<SurveyResponse> => {
    ...
    if (!response.ok) {
      const err = await response.json().catch(() => null);
      throw new Error(extractApiError(err, "Failed to update survey"));
    }
  ...
  delete: async (id: string, token: string): Promise<void> => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to delete survey"));
    }
  },
  ...
  updateStatus: async (id: string, status: string, token: string): Promise<SurveyResponse> => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to update survey status"));
    }
  ...
  bulkAssign: async (id: string, surveyMasterIds: string[], token: string): Promise<{ success: boolean; message?: string }> => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to assign survey"));
    }
  ...
  assignWithLocation: async (id: string, surveyMasterId: string, locationConstraint: any, token: string) => {
    ...
    if (!response.ok) {
      const err = await response.json().catch(() => null);
      throw new Error(extractApiError(err, "Failed to assign survey"));
    }
  ...
  getLocationConstraint: async (surveyId: string, surveyMasterId: string, token: string) => {
    ...
    if (!response.ok) {
      const err = await response.json().catch(() => null);
      throw new Error(extractApiError(err, "Failed to fetch location constraint"));
    }
  ...
  updateLocationConstraint: async (surveyId: string, surveyMasterId: string, constraint: any, token: string) => {
    ...
    if (!response.ok) {
      const err = await response.json().catch(() => null);
      throw new Error(extractApiError(err, "Failed to update location constraint"));
    }
  ...
  removeLocationConstraint: async (surveyId: string, surveyMasterId: string, token: string) => {
    ...
    if (!response.ok) {
      const err = await response.json().catch(() => null);
      throw new Error(extractApiError(err, "Failed to remove location constraint"));
    }
  ...
  getQuestions: async (token: string, surveyId: string) => {
    ...
    if (!response.ok) {
      const err = await response.json().catch(() => null);
      throw new Error(extractApiError(err, "Failed to fetch questions"));
    }
