...
export const menuService = {
  ...
  create: async (token: string, data: { name: string; title: string; description: string }) => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to create menu"));
    }
  ...
  update: async (token: string, id: string, data: { name?: string; title?: string; description?: string }) => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to update menu"));
    }
  ...
  updateStatus: async (token: string, id: string, status: string) => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to update status"));
    }
  ...
  delete: async (token: string, id: string) => {
    ...
    if (!response.ok || !result.success) {
      throw new Error(extractApiError(result, "Failed to delete menu"));
    }
